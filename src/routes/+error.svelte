<div class="bg-grun-dk p-10">
    <div class="bg-oliv-lt w-5/6 mx-auto p-10 rounded-xl border border-black">
        <h1>404-Seite nicht gefunden </h1>
        <div class="text-base">
            <p class="pb-8">Die angeforderte Seite konnte leider nicht gefunden werden. Bitte kehre zurück, versuche es erneut, und überprüfe, ob die URL korrekt eingegeben wurde. Unsere Techniker arbeiten daran, das Problem zu lösen. Wir danken dir für dein Verständnis.</p> 
        </div>
    </div>
    <div class="w-5/6 mt-5 mx-auto ">
        <a class="bg-oliv-lt py-2 inline-block px-10 mx-auto rounded-xl border border-black text-base" href="/">← Gehe Zurück, ziehe nicht über Los, löse kein 100 DM ein.</a>
    </div>
</div>